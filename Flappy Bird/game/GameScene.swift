...
class GameScene: SKScene {
    ...
    let githubButtonTexture = Assets.shared.sprites.textureNamed("leaderboard")
    ...
    let pauseButtonTexture = Assets.shared.sprites.textureNamed("pause")
    let resumeButtonTexture = Assets.shared.sprites.textureNamed("resume")
    ...
    func authenticateLocalPlayer() {
        ...
        localPlayer.authenticateHandler = { viewController, error in
            if let viewController = viewController {
                // Present the view controller if authentication is needed
                ...
                // You can perform additional setup here, like loading leaderboards
            } else {
    ...
    lazy var pauseButton: SKSpriteNode = {
        ...
        pauseButton.setScale(3.0)
        pauseButton.position = CGPoint(x: width / 2 - 130, y: 3 * height / 4 + 150)
        pauseButton.zPosition = GamezPosition.score + 1
        return pauseButton
    }()
    
    lazy var resumeButton: SKSpriteNode = {
        let resumeButton = SKSpriteNode(texture: resumeButtonTexture)
        resumeButton.name = "resumeButton"
        resumeButton.setScale(3.0)
        resumeButton.position = CGPoint(x: width / 2 - 130, y: 3 * height / 4 + 150)
        resumeButton.zPosition = GamezPosition.score + 1
    return resumeButton
}()
    ...
    lazy var bird = SKSpriteNode(texture: defaultBirdTexture.then { $0.filteringMode = .nearest }).then {
        ...
        $0.position = CGPoint(x: (width / 2), y: (height / 2 + 60) + 75)
    ...
    lazy var githubButton = SKSpriteNode(texture: githubButtonTexture.then { $0.filteringMode = .nearest }).then {
        $0.name = "github"
        $0.setScale(1.0)
        $0.position = CGPoint(x: (screenData.shared.width / 2) + 69, y: (screenData.shared.height / 2) - 115)
    }
    
    lazy var playButton = SKSpriteNode(texture: flappyplayButtonTexture.then { $0.filteringMode = .nearest }).then {
        $0.name = "play"
        $0.setScale(1.0)
        $0.position = CGPoint(x: (width / 2) - 69, y: (height / 2) - 115)
    }
    ...
    override func touchesBegan(_ touches: Set<UITouch>?, with event: UIEvent?) {
        ...
        if touchedNodeName == "play" && !GameScene.hitButton {
            ...
            run(SKAction.sequence([
                ...
                completion: {
                    guard let url = URL(string: "https://www.github.com/somerandomguythatneedshelp") else { return }
        ...
        } else if touchedNodeName == "pauseButton" {
            handlePauseButtonTouch()
        } else if touchedNodeName == "resumeButton" {
            handlePauseButtonTouch()
        } else if firstTouch {
    ...
    func handlePauseButtonTouch() {
        run(SKAction.sequence([
            SKAction.run { self.playSound(sound: self.swooshAction) },
            SKAction.run { self.pauseButton.setScale(1.15) },
            SKAction.wait(forDuration: 0.1),
            SKAction.run { if self.haptics { self.impact.impactOccurred() } },
            SKAction.run { self.pauseButton.setScale(1.2) },
            SKAction.run { self.togglePause() }
        ]))
    }
    ...
    func togglePause() {
        ...
        if isPaused {
            ...
            if resumeButton.parent == nil {
                addChild(resumeButton)
            }
            moving.isPaused = true
            bird.isPaused = true
            pipes.isPaused = true
        } else {
    ...
    func resetScene() {
        ...
        bird.speed = 1
        bird.zRotation = 0.0
        bird.position = CGPoint(x: width / 2.5, y: height / 2 + 60)
        bird.physicsBody?.do {
            $0.isDynamic = false
